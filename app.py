...
def predict_load_trend(data_list):
    ...
    return float(model.predict([[len(y) + 5]]))

# --- 4. CONFIGURAÇÃO VISUAL ---
st.set_page_config(page_title="XEON COMMAND v33.1", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""
    <style>
...
if 'intel_log' not in st.session_state: st.session_state.intel_log = collections.deque(["[SYSTEM ON] KERNEL v33.1 SOBERANO"], maxlen=12)
...
def run_sovereign_kernel(node, u_in=""):
    ...
    if node == "BIO_GEN": res = analyze_genetics_real(u_in)
        ...
        try:
            res = f"BTC: ${yf.Ticker('BTC-USD').fast_info.last_price:.2f}"
...
# --- 6. INTERFACE ---
st.write(f"📡 XEON NODE v33.1 | SOBERANIA REAL | {time.strftime('%H:%M:%S')}")
...
if st.session_state.is_locked:
    ...
    with c4:
        ...
        def generate_pdf():
            # CORREÇÃO DEFINITIVA DO VARIABLE NAME
            output = BytesIO() 
            p = canvas.Canvas(output, pagesize=A4)
            p.setFillColorRGB(0,0,0); p.rect(0,0,600,900,fill=1)
            p.setFillColorRGB(0,1,0.8); p.setFont("Courier-Bold", 14); p.drawString(50, 820, "RELATÓRIO SOBERANO v33.1")
...
def telemetry_fragment():
    ...
    st.write(f"📊 **HARDWARE:** CPU: {psutil.cpu_percent()}% | **TREND:** {prediction:.1f}% | **STATUS:** ✅ ACTIVE")
