...
def predict_load_trend(data_list):
    """Predição de carga via Regressão Linear (ML Real)"""
    if len(data_list) < 10: return 0.0
    try:
        y = np.array(data_list).reshape(-1, 1)
        x = np.arange(len(y)).reshape(-1, 1)
        model = LinearRegression().fit(x, y)
        prediction = model.predict([[len(y) + 1]])
        return float(prediction.flatten()[0]) # CORREÇÃO: Extração segura do escalar
    except: return 0.0

# --- 4. CONFIGURAÇÃO VISUAL ---
st.set_page_config(page_title="XEON COMMAND v34.0", layout="wide", initial_sidebar_state="collapsed")
st.markdown("""
    <style>
    ...
    .terminal { border: 1px solid #00FFCC; padding: 12px; background: #000; height: 180px; font-size: 11px; overflow-y: auto; color: #00FFCC; line-height: 1.4; border-style: double; }
...
if 'intel_log' not in st.session_state: st.session_state.intel_log = collections.deque(["[SYSTEM ON] KERNEL v34.0"], maxlen=12)
if 'hw_trace' not in st.session_state: st.session_state.hw_trace = collections.deque([random.uniform(2,5) for _ in range(60)], maxlen=100)
...
st.write(f"📡 XEON NODE v34.0 | REAL SOBERANIA | {time.strftime('%H:%M:%S')}")

if st.session_state.is_locked:
    st.error("❌ LOCKED BY SECURITY")
    ...
    u_query = st.text_input("", placeholder="INSERT DATA / SEARCH DNA / GEO SCAN (PT/EN)...", label_visibility="collapsed")
    if st.button("EXECUTE OMNI PROTOCOL"):
    ...
    with c4:
        ...
        def generate_pdf_final():
            ...
            p.setFillColorRGB(0,1,0.8); p.setFont("Courier-Bold", 14); p.drawString(50, 820, "REPORT SOBERANO v34.0")
            ...
            p.showPage(); p.save(); output.seek(0)
            return output
        st.download_button("📄 PDF REPORT", data=generate_pdf_final(), file_name="XEON_v34.pdf")

# LOGS E TELEMETRIA (FRAGMENTO PARA PERFORMANCE)
...
def telemetry_ui():
    ...
    pred = predict_load_trend(cpu_list)
