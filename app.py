...
import time, hashlib, sqlite3, os, urllib.parse
...
# CSS PARA VISUAL IDÊNTICO À IMAGEM
st.markdown("""
    <style>
...
class XeonEngineV13:
    ...
    def _init_db(self):
        """Inicializa DB e corrige colunas se necessário (MIGRATION)"""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("CREATE TABLE IF NOT EXISTS audit_ledger (ts REAL, payload BLOB)")
            # Tenta adicionar a coluna hash caso o DB seja de uma versão anterior
...
with c4:
    ...
    st.markdown('<button class="stButton btn-pdf">📄 PDF DE SOBERANIA</button>', unsafe_allow_html=True)
...
if not df.empty and len(df) >= 128:
    ...
    >> INTEGRIDADE SHA-256: {current_hash[:32]}...
