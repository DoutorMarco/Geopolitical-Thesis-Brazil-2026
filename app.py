...
st.markdown("""
    ...
    /* Estilização de Ingestão Verde/Ciano */
    textarea { 
        background-color: #000000 !important; 
        color: #00FF41 !important; 
...
# --- [2. MOTOR DE DOSSIÊ TÉCNICO (6 PÁGINAS)] ---
def generate_6_page_dossier(module_name):
    buf = BytesIO()
    p = canvas.Canvas(buf, pagesize=A4)
    setores = ["AUDITORIA NIST", "FINANÇAS GLOBAIS", "GOVERNANÇA PQC", "FISIOLOGIA DIGITAL", "EVIDÊNCIA EB-1A", "VEREDITO FINAL"]
    
    for i, setor in enumerate(setores):
        p.setFillColorRGB(0, 0, 0); p.rect(0, 0, 600, 900, fill=1)
        p.setFillColorRGB(0, 1, 0.25) # Verde Matrix
        p.setFont("Courier-Bold", 18)
        p.drawCentredString(300, 800, f"NEXUS SUPREMO v1032 - {module_name}")
        p.setFont("Courier", 12)
        p.drawString(50, 750, f"SETOR: {setor}")
        p.drawString(50, 730, f"ARQUITETO: MARCO ANTONIO DO NASCIMENTO")
        p.drawString(50, 710, f"ID OPERACIONAL: {hashlib.sha256(module_name.encode()).hexdigest()[:12].upper()}")
        p.drawString(50, 690, f"TIMESTAMP: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        p.drawString(50, 650, f"PÁGINA {i+1} DE 6 - MISSION CRITICAL DATA")
...
st.markdown("<h1 style='text-align: center; color: #38BDF8; letter-spacing: 8px;'>🛡️ NEXUS SUPREMO v1032</h1>", unsafe_allow_html=True)

# TELEMETRIA
col_a, col_b, col_c, col_d = st.columns(4)
with col_a: st.metric("CPU CARGA", f"{psutil.cpu_percent()}%")
with col_b: st.metric("MEMÓRIA", f"{psutil.virtual_memory().percent}%")
with col_c: st.metric("TRÁFEGO REDE", "214.5 MB")
with col_d: st.metric("STATUS SOH", "v2.2 ACTIVE")
...
with col_map:
    # Mapa Global 3D (Plotly Orthographic)
    ...
    map_fig.update_layout(
        geo=dict(
            ...
            countrycolor='#00FF41', projection_type='orthographic',
            showocean=False, showcountries=True
        ),
...
with col_terminal:
    ...
    cmd = st.text_area("Executar Auditoria Xeon...", height=120, label_visibility="collapsed")
    if st.button("🚀 PROCESSAR COMANDO"):
        res = f"Comando '{cmd[:20]}' em processamento. Integridade: 100%."
        st.session_state.last_res = res
        speak(res)

st.divider()

# MÓDULOS DE MISSÃO (9 NÓS)
...
for i, (label, key) in enumerate(btns):
    with cols[i % 3]:
        if st.button(label, key=f"btn_{key}", use_container_width=True):
            ...
            speak(f"Módulo {label} ativado. Gerando dossiê técnico.")
            st.session_state.pdf_buffer = generate_6_page_dossier(label)

if 'pdf_buffer' in st.session_state:
    st.download_button(
        label=f"📂 BAIXAR DOSSIÊ 6 PÁGINAS: {st.session_state.active_module}",
        data=st.session_state.pdf_buffer,
        file_name=f"Nexus_Audit_{st.session_state.active_module}.pdf",
        mime="application/pdf",
        use_container_width=True
    )

# PULSO DE SINCRONIA (ONDA REALTIME)
st.divider()
