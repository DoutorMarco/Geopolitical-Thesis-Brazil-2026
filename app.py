...
# --- [1. IDENTIDADE SOBERANA v340.0 - BLACKOUT TOTAL] ---
st.set_page_config(page_title="NEXUS SUPREMO v340.0", layout="wide", initial_sidebar_state="collapsed")
...
# --- [2. MOTOR DE DOSSIÊ COM ASSINATURA PQC] ---
def generate_quantum_dossier(module_name):
    buf = BytesIO()
    p = canvas.Canvas(buf, pagesize=A4)
    sections = [
        "I. NIST CYBERSECURITY INFRASTRUCTURE AUDIT",
        "II. GLOBAL FINANCIAL SYSTEMS (QUANTUM SECURED)",
    ...
    # Gerador de Hash Pós-Quântico para o Nó
    ...
    for i, title in enumerate(sections):
        ...
        p.drawString(50, 700, f"PQC SIGNATURE: {q_hash[:32]}")
        p.drawString(50, 640, f"PAGE {i+1} OF 6 | QUANTUM IMMUTABLE LEDGER")
...
st.markdown("<h1 style='text-align: center; color: #38BDF8; letter-spacing: 12px;'>🛡️ NEXUS SUPREMO v340.0</h1>", unsafe_allow_html=True)

c1, c2, c3, c4 = st.columns(4)
with c1: st.metric("CPU LOAD", f"{psutil.cpu_percent()}%")
with c2: st.metric("QUANTUM STATUS", "SECURED", delta="PQC ACTIVE")
with c3:
    try: 
...
with c4: st.metric("IPO VALUATION", "GOLD-ACTIVE")
...
with col_map:
    ...
    map_fig.update_layout(geo=dict(bgcolor='rgba(0,0,0,0)', showland=True, landcolor='#050505', projection_type='orthographic', projection_rotation=dict(lon=rotation, lat=20, roll=0)), margin=dict(l=0,r=0,t=0,b=0), height=450, paper_bgcolor='rgba(0,0,0,0)')
    st.plotly_chart(map_fig, use_container_width=True)

with col_cmd:
    st.write("### ⌨️ COMMAND INGESTION")
    query = st.text_area("Analyze Mission Parameters...", height=150, label_visibility="collapsed")
    if st.button("🚀 EXECUTE QUANTUM COMMAND"):
        speak(f"Quantum encryption active. Analyzing {query[:15]}. Results are now immutable.")

st.divider()

# --- [4. OS 9 NÓS DE MISSÃO CRÍTICA] ---
...
for i, (label, key) in enumerate(nodes):
    with cols[i % 3]:
        # Destaque especial para o nó IPO GOLD
        color_node = "#FFD700" if "IPO" in label else MATRIX_GREEN
        st.markdown(f"<div style='text-align:center; color:{color_node}; border-bottom:1px solid {color_node}; margin-bottom:5px;'>{label}</div>", unsafe_allow_html=True)
        if st.button(f"ACTIVATE {key}", key=f"node_{key}", use_container_width=True):
            speak(f"Node {label} engaged under Post-Quantum Cryptography.")
            st.session_state.current_pdf = generate_quantum_dossier(label)
            st.session_state.node_active = label

if 'current_pdf' in st.session_state:
    st.download_button(f"📥 DOWNLOAD {st.session_state.node_active} PQC DOSSIER", st.session_state.current_pdf, f"NEXUS_PQC_{st.session_state.node_active}.pdf", use_container_width=True)

# --- [5. ONDA DE PULSO OPERACIONAL REALTIME] ---
